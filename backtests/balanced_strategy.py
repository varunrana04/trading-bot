...
# C++ Engine integration for faster indicators
try:
...
if CPP_AVAILABLE:
    logger.info("Using C++ accelerated indicators")
...
class BalancedSignals:
    ...
    def prepare_15m(self, df: pd.DataFrame) -> pd.DataFrame:
        """Prepare 15m indicators - uses C++ when available for speed"""
        df = df.copy()
        ...
        if CPP_AVAILABLE:
            # Use C++ accelerated indicators (up to 170x faster)
            df['EMA_5'] = FastIndicators.ema(close, 5)
            df['EMA_13'] = FastIndicators.ema(close, 13)
            df['RSI'] = FastIndicators.rsi(close, 10)
            df['ATR'] = FastIndicators.atr(high, low, close, 10)
            
            # MACD using C++
            macd_result = FastIndicators.macd(close, 8, 17, 9)
            df['MACD'] = macd_result['macd_line']
            df['MACD_sig'] = macd_result['signal_line']
        else:
            # Fallback to Python
